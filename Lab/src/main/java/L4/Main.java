...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        walker.walk(listen, tree);//先遍历构建符号表
        MyTreeVisitor visitor = new MyTreeVisitor(listen.hashtable);
        visitor.visit(tree);
        visitor.codelist.print();
    }
}
class MyTreeVisitor extends CmmParserBaseVisitor{
    public HashTable hashtable;
    public InterCodeList codelist = new InterCodeList();
    public boolean isstruct = false;
    public int labelcount = 0;
    public int tempnumcount = 0;
    public MyTreeVisitor(HashTable hashtable){
        this.hashtable = hashtable;
    }
    @Override public InterCode visitFundec1(CmmParser.Fundec1Context ctx) {
        Operand operand = new Operand(OperandKind.FUNCTION,ctx.ID().getText());
        InterCode.MonoOpInterCode monoOpInterCode = new InterCode.MonoOpInterCode(CodeKind.FUNCTION,operand);
        codelist.setInterCodeTail(monoOpInterCode);
        List<CmmParser.ParamDecContext> contexts = ctx.varList().paramDec();
        for(CmmParser.ParamDecContext context:contexts){
            Operand tempoperand = new Operand(OperandKind.VARIABLE,context.varDec().ID().getText());
            InterCode.MonoOpInterCode code = new InterCode.MonoOpInterCode(CodeKind.PARAM,tempoperand);
            codelist.setInterCodeTail(code);
        }
        return monoOpInterCode;
    }
    @Override public InterCode visitFundec0(CmmParser.Fundec0Context ctx) {
        Operand operand = new Operand(OperandKind.FUNCTION,ctx.ID().getText());
        InterCode.MonoOpInterCode monoOpInterCode = new InterCode.MonoOpInterCode(CodeKind.FUNCTION,operand);
        codelist.setInterCodeTail(monoOpInterCode);
        return monoOpInterCode;
    }
    @Override public InterCode visitStructspf1(CmmParser.Structspf1Context ctx) {
        isstruct = true;
        return null;
    }
    @Override public InterCode visitParamDec(CmmParser.ParamDecContext ctx) {
        return null;
    }
    @Override public InterCode visitVarDec(CmmParser.VarDecContext ctx) {
        return null;
    }
    @Override public InterCode visitDef(CmmParser.DefContext ctx) {
        List<CmmParser.DecContext> contexts = ctx.decList().dec();
        for(CmmParser.DecContext context:contexts){
            if(context.ASSIGNOP()==null){
                if(context.varDec().getText().contains("[")){
                    Operand operand = new Operand(OperandKind.CONSTANT,context.varDec().ID().getText());
                    int size = ((Array) hashtable.get(context.varDec().ID().getText()).type).num*4;
                    InterCode.MemDecInterCode code = new InterCode.MemDecInterCode(CodeKind.DEC,operand,size);
                    codelist.setInterCodeTail(code);
                }
            }else{
    ...
    @Override public InterCode visitStmts(CmmParser.StmtsContext ctx) {
        if(ctx.exp()==null){
            visit(ctx.compSt().defList());
            visit(ctx.compSt().stmtList());
        }else{
            visit(ctx.exp());
        }
        return null;
    }
    @Override public InterCode visitStmtret(CmmParser.StmtretContext ctx) {
        InterCode.MonoOpInterCode expcode = (InterCode.MonoOpInterCode) visit(ctx.exp());
        Operand operand = expcode.operand;
        InterCode.MonoOpInterCode monoOpInterCode = new InterCode.MonoOpInterCode(CodeKind.RETURN,operand);
        codelist.setInterCodeTail(monoOpInterCode);
        return null;
    }
    @Override public InterCode visitStmtjug(CmmParser.StmtjugContext ctx) {
        if(ctx.IF()!=null){
            if(ctx.ELSE()==null){
    ...
    @Override public InterCode visitExpfunc(CmmParser.ExpfuncContext ctx) {
        if(ctx.ID().getText().equals("read")){
            String t = "t" + tempnumcount;
            tempnumcount++;
            Operand operand = new Operand(OperandKind.VARIABLE,t);
            InterCode.MonoOpInterCode monoOpInterCode = new InterCode.MonoOpInterCode(CodeKind.READ,operand);
            codelist.setInterCodeTail(monoOpInterCode);
            return monoOpInterCode;
        }else if(ctx.ID().getText().equals("write")){
            List<CmmParser.ExpContext> contexts = ctx.args().exp();
            for(CmmParser.ExpContext context:contexts){
                InterCode.MonoOpInterCode expcode = (InterCode.MonoOpInterCode) visit(context);
                Operand operand = expcode.operand;
                InterCode.MonoOpInterCode monoOpInterCode = new InterCode.MonoOpInterCode(CodeKind.WRITE,operand);
                codelist.setInterCodeTail(monoOpInterCode);
            }
            return null;
        }else{
            if(ctx.args()==null){
                ...
                for(int i=contexts.size()-1;i>=0;i--){
                    InterCode.MonoOpInterCode expcode = (InterCode.MonoOpInterCode) visit(ctx.args().exp(i));
                    Operand operand = expcode.operand;
                    if(hashtable.get(operand.value).type.k==Kind.STRUCTURE || hashtable.get(operand.value).type.k==Kind.ARRAY){
                        String t = "t" + tempnumcount;
                        tempnumcount++;
                        Operand operand1 = new Operand(OperandKind.VARIABLE,t);
                        InterCode.AssignInterCode code = new InterCode.AssignInterCode(CodeKind.ASSIGNADDRES,operand1,operand);
                        codelist.setInterCodeTail(code);
                        InterCode.MonoOpInterCode monoOpInterCode = new InterCode.MonoOpInterCode(CodeKind.ARG,operand1);
                        codelist.setInterCodeTail(monoOpInterCode);
                    }else{
                        InterCode.MonoOpInterCode code = new InterCode.MonoOpInterCode(CodeKind.ARG,operand);
                        codelist.setInterCodeTail(code);
                    }
                }
            }
    ...
    @Override public InterCode visitExpstruct(CmmParser.ExpstructContext ctx) {
        return null;
    }
    @Override public InterCode visitExpbin(CmmParser.ExpbinContext ctx) {
        //exp操作结束应该返回具体的operand
        return null;
    }
    @Override public InterCode visitExpopt(CmmParser.ExpoptContext ctx) {
        return null;
    }
    @Override public InterCode visitExpequ(CmmParser.ExpequContext ctx) {
        InterCode.MonoOpInterCode codeleft = (InterCode.MonoOpInterCode) visit(ctx.exp(0));
        InterCode.MonoOpInterCode coderight = (InterCode.MonoOpInterCode) visit(ctx.exp(1));

        return null;
    }
    @Override public InterCode visitExparr(CmmParser.ExparrContext ctx) {
        return null;
    }
    @Override public InterCode visitExpnot(CmmParser.ExpnotContext ctx) {
        return null;
    }
    @Override public InterCode visitExpid(CmmParser.ExpidContext ctx) {
        Operand operand = new Operand(OperandKind.VARIABLE,ctx.ID().getText());
        InterCode.MonoOpInterCode code = new InterCode.MonoOpInterCode(null,operand);
        return code;
    }
    @Override public InterCode visitExpval(CmmParser.ExpvalContext ctx) {
        String val = "#" + ctx.INT().getText();
        Operand operand = new Operand(OperandKind.CONSTANT,val);
        InterCode.MonoOpInterCode code = new InterCode.MonoOpInterCode(null,operand);
        return code;
    }
    @Override public InterCode visitExpnothing(CmmParser.ExpnothingContext ctx) {
        return (InterCode) visit(ctx.exp());
    }
}
...
abstract class InterCode {
    ...
    public static class MonoOpInterCode extends InterCode {
        Operand operand;
        public MonoOpInterCode(CodeKind kind,Operand operand){
            this.codeKind = kind;
    ...
    public static class BinOpInterCode extends InterCode {
        ...
        Operand operand2;
        Operand result;
        public BinOpInterCode(CodeKind kind,Operand fir,Operand sec,Operand res){
            this.codeKind = kind;
            this.operand1 = fir;
            this.operand2 = sec;
            this.result = res;
    ...
    public static class AssignInterCode extends InterCode {
        ...
        public AssignInterCode(CodeKind kind,Operand leftOperand,Operand rightOperand){
            this.codeKind = kind;
    ...
    public static class ConditionJumpInterCode extends InterCode {
        ...
        Operand operand2;
        Operand label;
        public ConditionJumpInterCode(CodeKind kind,Operand fir,Operand sec,String relop,Operand label){
            this.codeKind = kind;
            this.operand1 = fir;
            this.operand2 = sec;
            this.relop = relop;
    ...
    public static class MemDecInterCode extends InterCode {
        Operand operand;
        int size;
        public MemDecInterCode(CodeKind Kind,Operand operand,int size){
            this.codeKind = Kind;
            this.operand = operand;
            this.size = size;
            this.next = null;
...
class InterCodeList {
    ...
    public void setInterCodeTail(InterCode codeTail){
        if(interCodeHead==null) interCodeHead = codeTail;
        if(interCodeTail!=null) interCodeTail.next = codeTail;
        interCodeTail = codeTail;
    }
    public void print(){
        InterCode temp = interCodeHead;
        while(temp!=null){
            switch (temp.codeKind){
                case FUNCTION:
                    System.err.println(CodeKind.FUNCTION + " " + ((InterCode.MonoOpInterCode) temp).operand.value + " :");
                    break;
                case PARAM:
                    System.err.println(CodeKind.PARAM + " " + ((InterCode.MonoOpInterCode) temp).operand.value);
                    break;
                case DEC:
                    System.err.println(CodeKind.DEC + " " + ((InterCode.MemDecInterCode) temp).operand.value + " " + ((InterCode.MemDecInterCode) temp).size);
                    break;
                case RETURN:
                    System.err.println(CodeKind.RETURN + " " + ((InterCode.MonoOpInterCode) temp).operand.value);
                    break;
                case READ:
                    System.err.println(CodeKind.READ + " " + ((InterCode.MonoOpInterCode) temp).operand.value);
                    break;
                case WRITE:
                    System.err.println(CodeKind.WRITE + " " + ((InterCode.MonoOpInterCode) temp).operand.value);
                    break;
                default:
                    System.err.println(temp.codeKind);
                    break;
            }
            temp = temp.next;
        }
...
class CmmSemanticListener extends CmmParserBaseListener {
    ...
    @Override public void exitVarDec(CmmParser.VarDecContext ctx) {
        ...
        if(ctx.getText().contains("[")){
            int nums = Integer.parseInt(ctx.INT(0).getText());
            type = new Array(type,ctx.LB().size(),nums);
        }
        values.put(ctx,type);
    }
    ...
    @Override public void exitExparr(CmmParser.ExparrContext ctx) {
        ...
        if(type0.k!=Kind.ARRAY){
            ...
            if(arraytype.dim-1==0){
                ...
                values.put(ctx,new Array(arraytype.getArraytype(),arraytype.dim-1,0));
            }
        }
    }
...
class Array extends Type{
    ...
    int dim;
    int num;
    public Array(Type t,int d,int n){
        this.k=Kind.ARRAY;
        this.arraytype=t;
        this.dim=d;
        this.num=n;
